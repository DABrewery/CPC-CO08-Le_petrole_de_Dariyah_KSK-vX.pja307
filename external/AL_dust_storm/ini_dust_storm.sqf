...
null = [direction_duststorm, duration_duststorm, effect_on_objects, wall_of_dust, lethal_wall,visibility] execvm "external\AL_dust_storm\al_duststorm.sqf";
...
null = [340,600,false,false,false,0.] execvm "external\AL_dust_storm\al_duststorm.sqf";

// wind effect on objects surounding players ON, wall of dust OFF
null = [340,600,true,false,false,0.3] execvm "external\AL_dust_storm\al_duststorm.sqf";

// wind effect on objects surounding players ON, wall of dust ON, wall of dust non-destructive
null = [340,600,true,true,false,0.3] execvm "external\AL_dust_storm\al_duststorm.sqf";

// wind effect on objects surounding players ON, wall of dust ON, wall of dust destructive
null = [340,600,true,true,true,0.3] execvm "external\AL_dust_storm\al_duststorm.sqf";
...
if (isServer) then {
	while {al_duststorm_on} do {
		my_dust_storm_duration = 120;
		//publicVariable "my_dust_storm_duration";
		pause_between_dust_storm = 120;
		//publicVariable "my_dust_storm_duration";
		waitUntil {(!isNil "my_dust_storm_duration") and (!isNil "pause_between_dust_storm")};
		hint "script dust appelé";
		[340,my_dust_storm_duration,false,false,false] execVm "external\AL_dust_storm\al_duststorm.sqf";
