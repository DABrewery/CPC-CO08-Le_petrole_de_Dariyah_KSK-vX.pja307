...
params [
	"_direction_duststorm",
	"_duration_duststorm",
	["_effect_on_objects", false],
	["_dust_wall", false],
	["_lethal_wall", false],
	["_vizibility",[0.7,0.005,0]]
];

// _vizibility is a parameter, not a local/private variable
//private ["_vizibility","_stormsource","_x_dev","_y_dev"];
private ["_stormsource","_x_dev","_y_dev"];
...
//al_foglevel		= fog;
al_foglevel		= fogParams; //We use fog params to have a better multiplayer simulation (AI are sensitive to fog, but not to particles)
al_rainlevel	= rain;
...
//Loop that stops the dust script, tanin69's rewrite -> replaced by a loop : script must be stopped by changing al_duststorm_on value to false (and broadcast the variable if it is not changed on server side) 
//_stopDustScript = ["StopDustScript"] spawn {
[] spawn {
	while { true } do {
		waitUntil {!al_duststorm_on};
		
		hint "script dust arrêté par variable globale";

		//Restore initial mission parameters
		60 setFog [(al_foglevel#0), (al_foglevel#1), (al_foglevel#2)];;
...
// Original code by Alias Cartoons : stops the loop with _duration_duststorm variable
/*
...
//Server visibility/AI vision with AL's fog trick -> tanin69's rewrite -> https://community.bistudio.com/wiki/setFog
30 setFog [(_vizibility#0), (_vizibility#1), (_vizibility#2)];
...
// Launch local (player) dust effect
[[],"external\AL_dust_storm\alias_duststorm_effect.sqf"] remoteExec ["execVM"];

if (_dust_wall) then 
{
	_rand_pl = [] execVM "external\AL_dust_storm\alias_hunt.sqf";
	...
	[[_stormsource,_duration_duststorm,_x_dev,_y_dev],"external\AL_dust_storm\alias_dust_wall.sqf"] remoteExec ["execVM",0,true];
	...
	if (_lethal_wall) then {[_stormsource,_x_dev,_y_dev] execvm "external\AL_dust_storm\lethal_wall.sqf"};
...
// sets wind storm depending on direction
...
vx = floor (_unx * 0.6);
vy = (54 - vx)*fcty;
// incrementally increases the wind
inx = 0;
iny = 0;
incr = true;
incrx = false;
